...
export default {
    ...
    methods: {
        ...
        saveBarrageData(barrageText) {
            ...
            axios
                .post(
                    "http://luandou.gamemorefun.net/api/barrage/store",
                    params
                )
                .then(result => {
                    console.log("你好，弹幕插入成功了么状态", result.data);
                    this.$layer.msg("彈幕插入成功!");
                    that.options.barrageText = "";
                });
        },
        getHeroId(id, active) {
            console.log("正在点击的是当前的哪个图标id", id, active);
            this.heroId = id;
            if (active == true) {
                this.$layer.msg("您已经点亮过此武将了！");
            } else {
                active = true;
            }
        },
        getSharedHero() {
            ...
            axios
                ...
                .then(result => {
                    // console.log("有类型吗", result.data.data);
        ...
        heroFbShare() {
            ...
            if (this.heroId == "" || this.heroId == null) {
                ...
                axios
                    .post(
                        "http://luandou.gamemorefun.net/api/share/store",
                        params
                    )
                    .then(result => {
                        this.$layer.msg(
                        ...
                        // console.log("武将分享成功!", result.data);
                        that.options.flag = 1;
                    });
            }
        },
